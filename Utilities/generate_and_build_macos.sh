...
# generate_and_build_macos.sh
# Usage: ./generate_and_build_macos.sh [--action generate|build|all] [--build-dir DIR]
#        [--generator GEN] [--config Debug|Release] [--force] [--install] [--install-prefix DIR]
# This script is intended for macOS. It behaves like the Linux script but defaults
# the generator to "Unix Makefiles" (or "Ninja" if available).
...
INSTALL=false
INSTALL_PREFIX=""
JOBS=${JOBS:-$(sysctl -n hw.ncpu 2>/dev/null || nproc 2>/dev/null || echo 1)}
